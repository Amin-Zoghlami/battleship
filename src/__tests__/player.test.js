...
test("Players", () => {
  ...
  expect(realPlayer.attack(computerGameboard, 0, 0)).toBe(false);
  expect(realPlayer.attack(computerGameboard, 6, 4)).toBe(true);
  expect(() => realPlayer.attack(computerGameboard, -1, 5)).toThrow(
    "Coordinate (-1, 5) is out of bounds"
  );

  expect(computerPlayer.attack(realGameboard, 0, 0)).toBe(true);
  expect(realPlayer.attack(realGameboard, 6, 4)).toBe(false);
